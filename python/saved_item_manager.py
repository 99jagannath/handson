...
class SavedItemManager:
    ...
    def convert_dict_to_bytes(self, saved_item):
        res_bytes = json.dumps(saved_item).encode('utf-8')
        return res_bytes
    ...
    def replicate_dashboard_cross_region(self, base_region, region_list):
        ...
        for region in region_list:            
            ...
            for dashboard in dashboard_details:
                if not self.is_already_replicated(dashboard, replicated_dashboards, 'dashboard'):
                    if len(dashboard.saved_searches) > 0:
                        dashboard = self.update_saved_search_details(replicated_saved_searches, dashboard)
                    dashboard_dict = self.convert_management_type_obj_to_dict(dashboard)
                    create_exit_code = self.create_dashboard_in_region(dashboard_dict)
                    if create_exit_code == 0:
                        replication_count += 1
                    else:
